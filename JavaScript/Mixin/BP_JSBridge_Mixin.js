...
exports.BFL_JSLogic_Mixin = void 0;
...
class BFL_JSLogic_Mixin {
     ...
     */
    InitializeLogic(Target, LogicTypeName) {
    ...
    EmitEvent(EventName, Payload) {
        ...
        EventBus_1.EventBus.getInstance().emitScoped(EventName, -1, EventContext_1.GLOBAL_SCOPE, [parsedPayload]);
    }
    /**
     ...
     */
    EmitEventImmediate(EventName, Payload) {
        if (!EventName || EventName === "") {
        ...
        EventBus_1.EventBus.getInstance().emitImmediateScoped(EventName, -1, EventContext_1.GLOBAL_SCOPE, [parsedPayload]);
    }
    /**
     ...
     * @param Payload JSON 字符串格式的事件数据
     */
    EmitEventToInstance(LogicId, EventName, Payload) {
        if (!EventName || EventName === "") {
        ...
        const parsedPayload = BFL_JSLogic_Mixin._parsePayload(Payload);
        LogicManager_1.LogicManager.getInstance().emitToLogic(LogicId, EventName, [parsedPayload], { isHighFrequency: EventName === "OnTick" });
    }
    /**
     ...
     * @param Payload JSON 字符串格式的事件数据
     */
    EmitEventByOwner(Target, EventName, Payload) {
        if (!EventName || EventName === "") {
            console.warn(`[BFL_JSLogic] EmitEventByOwner 收到空事件名，忽略`);
            return;
        }
        const logicId = LogicManager_1.LogicManager.getInstance().findLogicIdByOwner(Target);
        if (logicId < 0) {
            // 降级为全局广播并警告
     ...
     */
    SetEventThrottle(EventName, IntervalMs) {
        EventBus_1.EventBus.getInstance().setThrottle(EventName, IntervalMs);
    }
    /**
     * 配置实例级事件节流（蓝图可调用）
     ...
     */
    SetInstanceEventThrottle(LogicId, EventName, IntervalMs) {
        if (LogicId < 0) {
            console.warn(`[BFL_JSLogic] SetInstanceEventThrottle 收到无效 LogicId=${LogicId}，忽略`);
            return;
        }
        EventBus_1.EventBus.getInstance().setScopedThrottle(EventName, LogicId, IntervalMs);
    }
    /**
     * 配置事件批处理（蓝图可调用）
     ...
     */
    SetEventBatch(EventName, WindowMs) {
        EventBus_1.EventBus.getInstance().setBatch(EventName, WindowMs);
    }
    /**
     * 设置事件调试模式（蓝图可调用）
     * 开启后会在控制台输出详细的事件分发日志
     ...
     * @param Enabled 是否启用
     */
    SetEventDebugMode(Enabled) {
        EventBus_1.EventBus.getInstance().setDebugMode(Enabled);
    }
    /**
     * 销毁逻辑
     *
    ...
    DestroyLogic(LogicId) {
        if (LogicId >= 0) {
            LogicManager_1.LogicManager.getInstance().destroyLogic(LogicId);
...
exports.BFL_JSLogic_Mixin = BFL_JSLogic_Mixin;
/**
 * 执行 Mixin 注入
 ...
 */
const BFL_JSLogic_C = (0, puerts_1.blueprint)("/Game/Blueprints/System/BFL_JSLogic.BFL_JSLogic_C");
if (BFL_JSLogic_C) {
    puerts_1.blueprint.mixin(BFL_JSLogic_C, BFL_JSLogic_Mixin);
...
//# sourceMappingURL=BP_JSBridge_Mixin.js.map