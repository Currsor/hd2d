...
exports.GLOBAL_SCOPE = exports.ScopeFilter = exports.EventBus = exports.EventPriority = void 0;
...
class EventBus {
    ...
    /** 事件名 → 批处理配置（全局级） */
    batchConfigs = new Map();
    /** "事件名@作用域" → 节流配置（实例级） */
    scopedThrottleConfigs = new Map();
    /** "事件名@作用域" → 批处理配置（实例级） */
    scopedBatchConfigs = new Map();
    /** 实例有效性校验函数（由 LogicManager 注册） */
    instanceValidator = null;
    /** 调试模式开关 */
    debugMode = false;
    /** 高频事件的调试日志抑制（避免日志洪泛） */
    suppressHighFrequencyLogs = true;
    constructor() { }
    /** 获取单例实例 */
    static getInstance() {
     ...
     */
    setDebugMode(enabled, suppressHighFrequency = true) {
    ...
    /** 注册实例有效性校验函数 */
    setInstanceValidator(validator) {
     ...
     * @returns 订阅记录
     */
    onScoped(eventName, subscriberId, handler, scopeOptions = EventContext_1.DEFAULT_SCOPE_OPTIONS, priority = exports.EventPriority.NORMAL) {
        ...
        if (subscriberId >= 0) {
            const duplicate = handlers.find(entry => entry.subscriberId === subscriberId && entry.handler === handler);
            if (duplicate) {
                if (this.debugMode) {
     ...
     */
    offBySubscription(subscription) {
     ...
     */
    offBySubscriberId(subscriberId) {
        let removedCount = 0;
     ...
     */
    emitScoped(eventName, senderId, scope, args, options) {
        this.emitWithScope(eventName, senderId, scope, args, options);
    }
    /**
     * 内部统一分发入口
     */
    emitWithScope(eventName, senderId, scope, args, options) {
        // 构建作用域化节流/批处理键
        const scopedKey = scope !== EventContext_1.GLOBAL_SCOPE ? `${eventName}@${scope}` : "";
        // 优先检查实例级节流/批处理
        if (scopedKey) {
     ...
     */
    emitImmediateScoped(eventName, senderId, scope, args, options) {
        const handlers = this.listeners.get(eventName);
        ...
        // 构建事件上下文
        const ctx = (0, EventContext_1.createEventContext)(eventName, senderId, scope, args, options);
        // 复制一份避免在回调中修改导致迭代问题
        ...
        for (const entry of handlersCopy) {
            ...
            // 作用域匹配过滤
            if (!(0, EventContext_1.matchesScope)(ctx, entry.subscriberId, entry.scopeOptions)) {
                continue;
            }
            matchedCount++;
            try {
     ...
     */
    setThrottle(eventName, intervalMs) {
     ...
     * @param intervalMs 最小触发间隔（毫秒）
     */
    setScopedThrottle(eventName, scope, intervalMs) {
     ...
     * @param scope 作用域（logicId）
     */
    removeScopedThrottle(eventName, scope) {
     ...
     * @param scope 作用域（logicId）
     */
    removeScopedConfigsByScope(scope) {
        const suffix = `@${scope}`;
     ...
     */
    emitThrottled(eventName, config, args, senderId = -1, scope = EventContext_1.GLOBAL_SCOPE, options) {
     ...
     * @param windowMs 批处理窗口时间（毫秒）
     */
    setScopedBatch(eventName, scope, windowMs) {
        const key = `${eventName}@${scope}`;
     ...
     */
    removeBatch(eventName, flush = true) {
    ...
    /** 批处理触发内部实现 */
    emitBatched(eventName, config, args, senderId = -1, scope = EventContext_1.GLOBAL_SCOPE, options) {
    ...
    /** 刷新批处理缓冲区（向后兼容，全局作用域） */
    flushBatch(eventName, config) {
        this.flushBatchScoped(eventName, config, -1, EventContext_1.GLOBAL_SCOPE);
    }
    /** 刷新批处理缓冲区（作用域化） */
    flushBatchScoped(eventName, config, senderId, scope, options) {
        config.timer = null;
        ...
        if (!handlers || handlers.length === 0)
            return;
        const ctx = (0, EventContext_1.createEventContext)(eventName, senderId, scope, batchedArgs, options);
        const handlersCopy = [...handlers];
        for (const entry of handlersCopy) {
            // 实例有效性校验
            if (entry.subscriberId >= 0 && this.instanceValidator) {
     ...
     */
    getListenerPriorities(eventName) {
        const handlers = this.listeners.get(eventName);
        if (!handlers)
            return [];
     ...
     */
    getSubscriberCount(eventName, subscriberId) {
        const handlers = this.listeners.get(eventName);
        if (!handlers)
            return 0;
...
// 导出上下文相关类型供外部使用
var EventContext_2 = require("./EventContext");
Object.defineProperty(exports, "ScopeFilter", { enumerable: true, get: function () { return EventContext_2.ScopeFilter; } });
Object.defineProperty(exports, "GLOBAL_SCOPE", { enumerable: true, get: function () { return EventContext_2.GLOBAL_SCOPE; } });
