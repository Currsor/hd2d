 ...
 * GameInstance 获取策略
 */
function getGameInstance(logTag) {
    // 策略1: puerts.argv（MainGame Start 传入）
    try {
        const gi = globalThis.puerts?.argv?.getByName?.("GameInstance");
        if (gi) {
    ...
    // 策略2: 全局引用
    try {
        const gi = globalThis.__gameInstance;
        if (gi) {
...
class SubsystemBridge {
    ...
    getSubsystem() {
        ...
        try {
            ...
            if (lib?.GetGameInstanceSubsystem) {
                const sub = lib.GetGameInstanceSubsystem(gi, subsystemClass);
                if (sub) {
     ...
     */
    getSubsystemClass() {
        if (this.subsystemClassCache)
            return this.subsystemClassCache;
        try {
...
exports.SubsystemBridge = SubsystemBridge;
//# sourceMappingURL=SubsystemBridge.js.map