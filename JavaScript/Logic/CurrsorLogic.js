...
const ComboAttackAbility_1 = require("../Ability/ComboAttackAbility");
/**
 * Currsor 角色专属逻辑类
 *
...
class CurrsorLogic extends CharacterBaseLogic_1.CharacterBaseLogic {
    ...
    Init(owner) {
        ...
        this.roleId = "Currsor";
        // 初始化冲刺能力
        this.dashAbility = new DashAbility_1.DashAbility(() => this.getOwnerAs(), 
        ...
        // 初始化三段连击能力
        this.comboAttack = new ComboAttackAbility_1.ComboAttackAbility(() => this.getOwnerAs(), 
        // 可在此覆盖默认配置
        ...
        // 生命周期回调
        {
            onAttackStart: (comboIndex) => {
                // 通知动画层播放对应段的攻击动画
                this.emitGlobal(EventTypes_1.EventTypes.OnComboAttackStart, comboIndex);
            },
            ...
            onComboEnd: (totalHits) => {
                this.emitGlobal(EventTypes_1.EventTypes.OnComboEnd, totalHits);
            },
        }, "CurrsorCombo");
    ...
    OnSetup() {
        ...
        this.subscribeScoped(EventTypes_1.EventTypes.OnDash, this.onDash.bind(this), {
            filter: EventContext_1.ScopeFilter.ANY,
        });
        // 订阅攻击输入事件（由 C++ Attack() → 蓝图 OnAttackTriggered → EmitEventByOwner 发出）
        this.subscribeScoped(EventTypes_1.EventTypes.OnAttack, this.onAttack.bind(this), {
            filter: EventContext_1.ScopeFilter.ANY,
        });
        // 订阅蓝图动画通知回传事件（由 PaperZD 动画通知 → EmitEvent 发出）
        this.subscribeScoped(EventTypes_1.EventTypes.OnAttackHitStart, this.onAttackHitStart.bind(this), {
            filter: EventContext_1.ScopeFilter.ANY,
        });
        this.subscribeScoped(EventTypes_1.EventTypes.OnAttackHitEnd, this.onAttackHitEnd.bind(this), {
            filter: EventContext_1.ScopeFilter.ANY,
        });
        this.subscribeScoped(EventTypes_1.EventTypes.OnComboWindowOpen, this.onComboWindowOpen.bind(this), {
            filter: EventContext_1.ScopeFilter.ANY,
        });
        this.subscribeScoped(EventTypes_1.EventTypes.OnComboWindowClose, this.onComboWindowClose.bind(this), {
            filter: EventContext_1.ScopeFilter.ANY,
        });
    }
    /**
    ...
    onDash() {
        if (!this.isActive)
            return;
        // 攻击中不允许冲刺（可根据需求调整）
        if (this.comboAttack.isAttacking)
            return;
